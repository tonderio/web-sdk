...
`getApmBanks`). The SDK returns the backend body as a raw passthrough
...
shape as `getTransaction`/`pollTransaction` — with no wrapper object and no
...
`pollTransaction`, `getPaymentMethods`, `getApmBanks`) MUST return the
...
- GIVEN a caller receives a result from `pay`, `getTransaction`, or
  `pollTransaction`
...
returned by `getTransaction` and `pollTransaction` (subject only to
...
`getTransaction` and `pollTransaction` MUST return the raw transaction
...
#### Scenario: Polling a transaction to its current state

- GIVEN a caller polls a transaction still in progress
- WHEN `pollTransaction` returns its current snapshot
- THEN the result is the raw transaction body at that point in time, with
  no wrapper object around it
