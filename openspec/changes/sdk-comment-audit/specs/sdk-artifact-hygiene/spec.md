...
surface. A comment on a `private` class member is not exempt: the
declaration emitter used by this SDK's build documents `private` member
shapes in the `.d.ts` output, so such a comment ships exactly as a `public`
member's does. A comment in a file that ships to no artifact is not exempt
...
- GIVEN the PROTECTED locations above, after any edit that rewrites or removes
