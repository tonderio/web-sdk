...
#### Scenario: Instance surface is empty
...
#### Scenario: Internal collaborators are unreachable
...
#### Scenario: Mutating secure_token after construction has no effect
...
#### Scenario: Mutating customer after a successful pay() has no effect
...
construction, never the mutated value.

This is the hardest case, and the reason it is stated separately: a "memoize
on first read" implementation satisfies every other scenario here and still
fails this one. If any guarded value is resolved lazily rather than at
construction, a mutation landing in the window before the first call is the
value that gets memoized.

#### Scenario: Pre-first-call mutation of customer does not leak into the first request
...
#### Scenario: events present at construction, later replaced — regression guard

- GIVEN `createTonder()` with `events.payment.on_completed = h1`
- WHEN the merchant sets `cfg.events.payment.on_completed = h2` before a
  payment resolves
- THEN `h2` fires and `h1` does not

#### Scenario: events absent at construction, added afterwards — regression guard

- GIVEN `createTonder()` with no `events` key at all
- WHEN the merchant assigns `cfg.events = { payment: { on_completed: h1 } }`
