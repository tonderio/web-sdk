...
/** ServiceManager key for the Direct API process service. */
...
export class Tonder {
  ...
  public async pay(input: PayInput): Promise<RawTransaction> {
    ...
    // payment method (backend requires customer on all /process operations).
    ...
    try {
      raw = await this.directApiService.processPayment(
        body,
  ...
  /**
   * Resolve the `/process` `payment_method` block for the requested method.
   *
  ...
  private async rollbackAutoEnrolledCard(
    ...
    try {
      ...
      // Best-effort rollback: the original /process error is the one that matters.
    }
  }
  ...
  /**
   * Build the `/process` body. Customer is sent INLINE (no customer call).
   * `client_reference` is business reconciliation data and remains in the body;
   * `idempotency_key` is transport-only and is mapped to `X-Request-Id` by
   * DirectApiService, never serialized into this body.
   */
  private buildProcessBody(
    ...
    // sent to /process (phone is intentionally never forwarded to the charge).
