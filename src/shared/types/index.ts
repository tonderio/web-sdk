 ...
 * `getPaymentMethodBanks()`. SafetyPay expects the bank routing `code` inside
 * `payment_method.config.bank_ids`, using the legacy shape `{ id: bank.code }`.
 */
export interface PaymentMethodBank {
  /** Bank identifier to pass in `config.bank_ids`. */
  id: number;
  /** Human-readable bank name. */
  name: string;
  /** Bank routing code used by SafetyPay in `config.bank_ids[].id`. */
  code: string;
...
export interface PayInput {
  ...
  /** Optional idempotency key sent as `X-Request-Id` for `/api/v1/process/` only. */
